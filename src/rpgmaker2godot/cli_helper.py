"""Helper routines for the CLI entry point (see :mod:`cli`).

Everything that is not the ``main`` function lives here: the argument
parser, the pipeline steps of both conversion modes (tileset and
character) and the small rendering helpers they share.
"""
...
# A2/A3/A4 sheet never produces a single, unboundedly long line.
